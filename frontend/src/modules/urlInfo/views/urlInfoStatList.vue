		...
		<article>
			...
			<div class="row">
                ...
                <li
                  ...
                  :key="idx"
                  :class="serchCond.periodMonth === item ? 'active' : ''"
                >
                  <a 
                    href="#" 
                    :title="`${item}개월 등록일자 검색`"
                    @click.prevent="serchCond.periodMonth = item"
                  >{{ item }}개월</a>
                </li>
							</ul>
							<a 
              ...
              >
                ...
                >{{ item }}개씩 보기</option>
                  ...
                  >
										...
										<td>{{ item.orgUrl }}</td>
										<td class="text-center">{{ `${$moment(item.regDt).format('L')}/${$moment(item.expDt).format('L')}` }}</td>
										<td class="text-right">{{ item.totCnt }}</td>
										<td class="text-right">{{ item.conCnt }}</td>
										<td class="text-right">{{ Math.trunc(item.conCnt / item.totCnt * 100) }}%</td>
                      ...
                      <a 
                        ...
                        title="선택"
                        @click.prevent="goUrl(item.urlId)"
                      >상세보기</a>
                    </td>
									</tr>
								</tbody>
...
export default {
  ...
  watch: {
		'pageInfo.listSize' (curVal, oldVal) {
  ...
  methods: {
    ...
    goUrl(urlId) {
      const url = `${this.$router.currentRoute.path}/${urlId}`
