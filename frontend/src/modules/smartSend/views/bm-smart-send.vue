    ...
    <div class="col-xs-12 consolMarginTop">
      <div class="of_h inline">
        <div class="float-right">
          <router-link :to="{ name: 'smartTemplate'}" tag="a" class="btnStyle2 borderGray"  activity="READ">스마트 발송 템플릿 관리<i class="fal fa-arrow-to-bottom" ></i></router-link>
          <a @click="fnProcSmartSend()" class="btnStyle2 backBlack ml10" title="스마트 메시지 발송" activity="SAVE">스마트 메시지 발송</a>
        </div>
      </div>
          ...
          <table class="table_skin1 bt-000 tbl-striped">
            ...
            <tbody>
              <tr v-for="(data, idx) in datas" :key="data.row_num">
                ...
                <td>{{ idx + 1 }}</td>
                <td class="text-center"><router-link :to="{ name: 'smartSendManage', params: {'tmpltCodeP': data.tmpltCode }}">{{data.tmpltCode}}</router-link> </td>
...
import smartSendApi from '../service/smartSendApi';
import smartTemplateApi from '../../smartTemplate/service/smartTemplateApi';
...
export default {
  ...
  methods: {
    ...
    async fnProcSmartSend(){
      ...
      this.$router.push({name: 'smartSendManage', params: {'tmpltCodeP': this.chkBox }});
    },
