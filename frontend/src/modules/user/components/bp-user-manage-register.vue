			...
			<div class="modal-content">
				<div class="modal-body">
					<div>
						...
						<div class="of_h">
							...
							<div style="width:80%" class="of_h inline-block float-right">
								<input type="text" v-model="loginId" @change="fnChangeUserId" class="inputStyle" style="width:70%" title="사용자 ID 입력란">
						...
						<div class="of_h consolMarginTop">
							...
							<input type="text" v-model="userName" class="inputStyle float-right" style="width:80%" title="사용자 명 입력란">
						</div>
						<div class="of_h consolMarginTop">
							<h5 class="inline-block" style="width:20%">휴대폰 번호*</h5>
							<input type="text" v-model="hpNumber" class="inputStyle float-right" style="width:80%" title="휴대폰 번호 입력란">
						</div>
						<div class="of_h consolMarginTop">
					...
					<div class="text-center mt20">
						<a @click="fnRegisterUser()" class="btnStyle1 backBlack mr5" title="등록">등록</a>
...
export default {
	...
	methods: {
		...
		//중복체크
		fnCheckDupcUser() {
			console.log(">>> start fnCheckDupcUser.");

			// 사용자ID 입력체크
			if(this.loginId == "" || this.loginId == null) {
				confirm.fnAlert("", "사용자ID를 입력하세요.");
				return false;
			}
			...
			userApi.checkDupcUser(params).then(response =>{
				...
				if(result.success) {
					confirm.fnAlert("", "사용 가능한 사용자ID 입니다.");
					this.isDupcUser =  true;
				} else {
					confirm.fnAlert("", "사용할 수 없는 사용자ID 입니다.");
					this.isDupcUser = false;
				}
			});
			console.log(">>> end fnCheckDupcUser.");
		},
		//사용자 등록
		fnRegisterUser() {
			console.log("fnRegisterUser Start");
			...
			var params = {
				...
				"roleCd"	: this.roleCd,
				"regId"		: tokenSvc.getToken().principal.loginId,
			...
			userApi.registerUser(params).then(response =>{
				var result = response.data;
				if(result.success) {
					confirm.fnAlert("", "사용자 등록에 성공했습니다.");
		...
		fnIsDupcChk() {
			if(!this.isDupcUser) {
				confirm.fnAlert("", "사용자ID 중복체크를 확인해주세요.");
				return false;
			}
			return true;
		},
