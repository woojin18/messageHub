...
//						, resultMsg=입력한 유치자( juoh )에 해당하는 대리점 정보가 존재하지 않습니다., resultList=null}}
	...
	public void saveDept(Map<String, Object> params) throws Exception {
		...
		if (isNew) {
			...
			// 청구번호가 기존과 상이할 경우 : 가입번호를 새로 발급 받는다.  
			if (!((String)params.get("orgBillId")).equals((String)params.get("billId"))) {
				Map bill = (Map) generalDao.selectGernalObject("dashboard.selectBill", params);
				if (bill != null) {
					AesEncryptor encrypt = new AesEncryptor(); // 암호화
					if (StringUtils.isNotEmpty((String) bill.get("napJumin"))) {
...
//		String regNo	= CommonUtils.getString(generalDao.selectGernalObject("project.selectRegNoForSaveProject", params));
		...
		joinMap.put("custNo",		custNo);   // 명의자 고객번호
		joinMap.put("logid",		params.get("corpId") + "_BC_" + params.get("deptCode"));  // 가입인식번호1(본인 서비스의 유니크 아이디)
		joinMap.put("indcId",		salesId);  // 유치자아이디(영업마당아이디)
		joinMap.put("mngrId",		salesId);  // 관리자아이디(영업마당아이디)
		...
		// 유큐브 서비스 등록 시 청구정보 객체입니다.
		ucubeBillInfoVo.put("billAcntNo",	params.get("billId"));										// 청구계정 번호 (등록시엔 공백으로 입력)
		ucubeBillInfoVo.put("billEmail",	params.get("billEmail"));					// 청구이메일주소 ?
		ucubeBillInfoVo.put("billKind",		params.get("billKind"));					// 청구서유형코드(Y=E-MAIL(상세)청구서, P=휴대폰(간편)청구서, C=MMS청구서(TEXT형태), N=우편(엽서요약) 청구서)
		ucubeBillInfoVo.put("billRegNo",	params.get("billRegNo"));					// 청구사업자번호
		ucubeBillInfoVo.put("billZip",		params.get("billZip"));						// 청구우편번호
		ucubeBillInfoVo.put("billJuso",		params.get("billJuso"));					// 청구주소
		ucubeBillInfoVo.put("billJuso2",	params.get("billJuso2"));					// 청구주소상세
		
		Map<String, Object> ucubePymInfoVO = new HashMap<>();
		
		ucubePymInfoVO.put("napCustKdCd",	params.get("napCustKdCd"));					// 납부자고객구분코드
		ucubePymInfoVO.put("napCmpNm",		params.get("napCmpNm"));					// 납부자명 
		ucubePymInfoVO.put("napJumin",		params.get("napJumin"));					// 생년월일/사업자번호
		ucubePymInfoVO.put("pymMthdCd",		params.get("payMthdCd"));					// 납부방법
		ucubePymInfoVO.put("cardNo",		params.get("cardNo"));						// 카드번호
		ucubePymInfoVO.put("cardValdEndYymm",params.get("cardValdEndYymm"));			// 카드유효기간
		ucubePymInfoVO.put("cardCd",		params.get("cardCd"));						// 카드코드
		ucubePymInfoVO.put("cardCmpName",	params.get("cardCmpName"));					// 카드사명
		ucubePymInfoVO.put("bankNm",		params.get("bankNm"));						// 은행명
		ucubePymInfoVO.put("bankNo",		params.get("bankNo"));						// 계좌번호
		ucubePymInfoVO.put("bankCd",		params.get("bankCd"));						// 은행코드
...
//					 }
		...
		if( "10000".equals(result.get("code")) ) {
			...
			boolean isBill = params.get("billId") == null;
